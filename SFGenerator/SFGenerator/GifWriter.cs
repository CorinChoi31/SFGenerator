    ...
    public void WriteFrame(Image Image, int Delay = 0)
    {
        lock (_syncLock)
            using (var gifStream = new MemoryStream())
            {
                ...
                ImageCodecInfo jpgEncoderInfo = GetEncoder(ImageFormat.Jpeg);
                EncoderParameters jpgEncoderParameters = new EncoderParameters(1);
                jpgEncoderParameters.Param[0] = new EncoderParameter(Encoder.Quality, 100L);
                Image.Save(jpgStream, jpgEncoderInfo, jpgEncoderParameters);
                //jpgEncoderParameters.Dispose();

                Bitmap bitmap = new Bitmap(jpgStream);
                //jpgStream.Dispose();

                ImageCodecInfo gifEncoderInfo = GetEncoder(ImageFormat.Gif);
                EncoderParameters gifEncoderParameters = new EncoderParameters(2);
                gifEncoderParameters.Param[0] = new EncoderParameter(Encoder.Compression, (long)EncoderValue.CompressionNone);
                gifEncoderParameters.Param[1] = new EncoderParameter(Encoder.Quality, 100L);
                bitmap.Save(gifStream, gifEncoderInfo, gifEncoderParameters);
                //gifEncoderParameters.Dispose();
                //bitmap.Dispose();
